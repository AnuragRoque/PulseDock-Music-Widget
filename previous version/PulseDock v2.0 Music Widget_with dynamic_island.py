...
try:
    import winreg  # Windows-only; used for the "Launch on startup" toggle
except ImportError:
    winreg = None
...
def _startup_command() -> str:
    ...
    Prefers the frozen .exe (PyInstaller build); when running from source it
    falls back to pythonw.exe + this script so no console window appears.
    """
...
def set_launch_on_startup(enabled: bool) -> bool:
    """Add/remove the Run-key value so the widget starts with Windows."""
    if winreg is None:
        return False
    try:
...
class Snapshot:
    ...
    can_prev: bool = True
    can_next: bool = True
    can_seek: bool = False  # source supports setting the playback position
    aumid: str = ""
    thumb: bytes = b""
...
_PIN_ICON_CACHE: dict = {}


def make_pin_icon(pinned: bool, color_hex: str) -> QIcon:
    """Pushpin icon drawn in code — upright when pinned, tilted 45° when not."""
    cache_key = (pinned, color_hex)
    ...
    for px in (16, 20, 24, 32, 48, 64):
        ...
        p.scale(s, s)
        if not pinned:
            # Tilted like the taskbar's "not pinned yet" affordance
            p.translate(32, 32)
            p.rotate(45)
            p.translate(-32, -32)
        ...
        p.setBrush(color)
        # Grip (head), flange, and needle on a 64x64 design grid
        head = QPainterPath()
        head.addRoundedRect(QRectF(23, 6, 18, 20), 4, 4)
        p.drawPath(head)
        flange = QPainterPath()
        flange.addRoundedRect(QRectF(17, 25, 30, 7), 2, 2)
        p.drawPath(flange)
        needle = QPainterPath()
        needle.moveTo(28, 32)
        needle.lineTo(36, 32)
...
async def read_snapshot_async(thumb_cache: Optional[dict] = None,
    ...
    try:
        ...
        pos_sec, dur_sec, start_sec = -1.0, -1.0, 0.0
        try:
        ...
        # Album art: only re-read the stream when the track changes — not on
        # every 1s poll. BUT: right after a track change browsers hand over
        # stale art (previous track's, or their own logo) because the title
        # updates before the art stream does — sometimes taking several
        # seconds. So a track's art is only trusted once it is older than a
        # settle window; until then every poll re-reads and replaces it. The
        # UI only repaints when the bytes actually change, so the extra reads
        # are invisible and cost ~8 local stream reads per track.
        ART_SETTLE_SEC = 8.0
        track_key = (title, artist, aumid)
        same_track = thumb_cache is not None and thumb_cache.get("key") == track_key
        cache_ok = (
            same_track
            and thumb_cache.get("data")
            and time.monotonic() - thumb_cache.get("first_seen", 0.0) > ART_SETTLE_SEC
        )
        if cache_ok:
            thumb = thumb_cache["data"]
            thumb_lowres = thumb_cache.get("lowres", False)
        else:
            fresh = await _read_thumbnail_async(props)
            if not fresh and same_track and thumb_cache.get("data"):
                # Stream hiccup — keep the art we already have for this track
                thumb = thumb_cache["data"]
                thumb_lowres = thumb_cache.get("lowres", False)
            else:
                thumb = fresh
                ...
                if thumb_cache is not None:
                    if not same_track:
                        thumb_cache["first_seen"] = time.monotonic()
...
async def send_seek_async(position_sec: float) -> bool:
    """Set the current session's playback position (seconds -> 100ns ticks)."""
    if MediaManager is None:
        return False
    try:
        manager = await MediaManager.request_async()
        ...
        ticks = int(max(0.0, position_sec) * 10_000_000)  # WinRT TimeSpan units
        result = method(ticks)
        if result is not None and hasattr(result, "__await__"):
            await result
        elif asyncio.iscoroutine(result):
...
# --- Win32 window helpers for "pin to window" -------------------------------
def _fg_hwnd() -> int:
    try:
...
def _shell_hwnd() -> int:
    try:
        return int(ctypes.windll.user32.GetShellWindow() or 0)
    except Exception:
        return 0
...
def _hwnd_class(hwnd: int) -> str:
    try:
        ...
        ctypes.windll.user32.GetClassNameW(ctypes.c_void_p(hwnd), buf, 64)
        return buf.value
    except Exception:
        return ""


# Desktop / taskbar host windows — never valid pin targets
_SHELL_CLASSES = {"Progman", "WorkerW", "Shell_TrayWnd", "Shell_SecondaryTrayWnd"}


def _is_pinnable_window(hwnd: int, my_pid: int) -> bool:
    """A real app window: alive, titled, not ours, not the desktop/taskbar."""
    return (
        bool(hwnd)
        and hwnd != _shell_hwnd()
        and _hwnd_alive(hwnd)
        and _hwnd_pid(hwnd) != my_pid
...
class ProgressLine(QWidget):
    ...
    LINE_H = 3
    HOVER_H = 5          # thicker bar while the seek strip is hovered/dragged
    KNOB_R = 5.0         # draggable play-head dot shown on hover
    ...
    def paintEvent(self, event):
        ...
        p.setClipPath(clip)
        line_h = self.HOVER_H if self._hover else self.LINE_H
        y = self.height() - line_h
        p.fillRect(QRectF(0, y, self.width(), line_h), self._track)
        fill_w = self.width() * self._fraction
        p.fillRect(QRectF(0, y, fill_w, line_h), self._fill)
        if self._hover:
            # Grabbable knob at the play head signals the bar is draggable
            cy = self.height() - line_h / 2.0
            p.setPen(Qt.PenStyle.NoPen)
            p.setBrush(self._fill)
            p.drawEllipse(QPointF(fill_w, cy), self.KNOB_R, self.KNOB_R)
        p.end()
...
class MusicWidget(QWidget):
    def __init__(self):
        ...
        # Click-to-seek / drag / album-art-click interaction state
        self._seeking = False
        self._seek_preview_frac = 0.0
        self._did_drag = False
        self._press_on_art = False
        self._press_global = QPoint()
        self._last_open_ts = 0.0  # debounce for click-artwork-to-open

        # Pin-to-window state (session only — window handles don't persist)
        self._pin_target_hwnd = 0     # 0 = not pinned
        self._pin_target_pid = 0
        self._pin_target_title = ""
        self._pin_forced_on_top = False
        self._last_ext_hwnd = 0       # most recent non-PulseDock foreground window
        self._current_dot_color = "#1DB954"
        ...
        # Foreground-window watcher: tracks what to pin to, and while pinned
        # enforces "show with the target window, hide otherwise". 60ms keeps
        # the attach/detach feeling instant (a handful of cheap Win32 calls).
        self._pin_timer = QTimer(self)
        self._pin_timer.timeout.connect(self._pin_tick)
        self._pin_timer.start(60)
        ...
        # Island auto-expand on track change collapses again after a moment
        self._island_auto_timer = QTimer(self)
        self._island_auto_timer.setSingleShot(True)
        self._island_auto_timer.timeout.connect(self._island_auto_collapse)

        # Debounced settings save (moveEvent fires per pixel while dragging)
        ...
        # Pin re-shows must not steal focus from the pinned window
        ...
        # One-time hint so the invisible gestures are discoverable
        if not self.seen_intro:
            QTimer.singleShot(1500, self._show_intro)
    ...
    def _build_ui(self):
        ...
        # Pin-to-window button just left of minimize: the widget follows one
        # window's fate (visible with it, hidden when it's minimized/covered)
        self.pin_btn = QToolButton(self.card)
        self.pin_btn.setObjectName("pin_btn")
        self.pin_btn.clicked.connect(self.toggle_pin)
        self.pin_btn.setToolTip("Pin to the active window")
    ...
    def _rebuild_card_layout(self, scale: int):
        ...
        if scale == 50:
            ...
            self.pin_btn.setIconSize(QSize(8, 8))
        elif scale == 100:
            self.min_btn.show()
            ...
            self.pin_btn.setIconSize(QSize(9, 9))
        else: # 150
            self.min_btn.show()
    ...
    def resizeEvent(self, event):
        ...
        if hasattr(self, "bg_label"):
            ...
            # Re-render the blurred art at the new size, otherwise the old
            # pixmap's baked-in rounded corners show as square edges while
            # the island expand/collapse animation runs (sig-cached: free
        ...
        # Pin button immediately to the left of minimize
        self.pin_btn.setGeometry(
            self.card.width() - x_offset - btn_w - 2,
            y_offset,
            btn_w,
            btn_h
        )
    ...
    def _build_tray(self):
        ...
        # Pin to the last-used window (same as the pin button; XS has no button)
        self.pin_menu_action = QAction("Pin to Active Window", self, checkable=True)
        self.pin_menu_action.setChecked(False)
        self.pin_menu_action.triggered.connect(self.toggle_pin)
        menu.addAction(self.pin_menu_action)

        # Launch with Windows (HKCU Run key)
        self.launch_startup_action = QAction("Launch on Startup", self, checkable=True)
        ...
        # Dynamic Island options: which existing layouts it uses at rest and
        # on hover
        island_menu = QMenu("Dynamic Island", self)
        ...
        for label, pct in [("Extra Small (XS)", 50), ("Small (S)", 75)]:
            ...
            self.island_idle_group.addAction(action)
            island_menu.addAction(action)
            self.island_idle_actions[pct] = action
        active_idle = self.island_idle_actions.get(self.island_idle)
        if active_idle:
            active_idle.setChecked(True)
        ...
        for label, pct in [("Normal (N)", 100), ("Mini Card (M)", 125)]:
            ...
            self.island_expand_group.addAction(action)
            island_menu.addAction(action)
            self.island_expand_actions[pct] = action
        active_expand = self.island_expand_actions.get(self.island_expand)
        if active_expand:
            active_expand.setChecked(True)

        menu.addMenu(island_menu)
    ...
    def toggle_launch_on_startup(self):
        enabled = self.launch_startup_action.isChecked()
        if set_launch_on_startup(enabled):
            self.launch_on_startup = enabled
            self.save_settings()
        else:
            # Registry write failed — revert the checkbox and warn
            self.launch_startup_action.setChecked(self.launch_on_startup)
    ...
    def _show_intro(self):
        try:
            self.tray.showMessage(
                APP_NAME,
                "Scroll to change volume  •  Right-click for themes & settings  •  "
                "Drag to move  •  Click the artwork to open your player",
    ...
    def _apply_scale(self):
        ...
        # touching the user's saved Always-on-Top preference.
        if island and not self._island_on_top and not self.always_on_top:
            self._island_on_top = True
            self._set_on_top_hint(True)
        elif not island and self._island_on_top:
            self._island_on_top = False
            if not self._pin_forced_on_top and not self.always_on_top:
                self._set_on_top_hint(False)

        if island:
            self._island_expanded = False
        new_w, new_h = self._scale_window_size(self.island_idle if island else scale)
        ...
        self._rebuild_card_layout(self.island_idle if island else scale)
    ...
    def _animate_geometry(self, target: QRect):
        ...
        anim.valueChanged.connect(self._geo_anim_step)
        anim.finished.connect(self._after_geo_anim)
        self._geo_anim = anim
        anim.start()
    ...
    def change_island_expand_from_action(self):
        action = self.sender()
        if not action:
            return
    ...
    def _apply_theme_style(self):
        ...
        # 3. Volume/pin icons + album-art placeholder follow the theme colors
        self._current_icon_color = icon_color
        self._current_dot_color = dot_color
    ...
    def _handle_snapshot(self, snap: Snapshot):
        if not isinstance(snap, Snapshot):
            return
        self.snapshot = snap
        # Remember the active source so "click artwork" can reopen it later
        if snap.aumid:
        ...
        if (
            ...
            self._set_island_expanded(True)
            self._island_auto_timer.start(3000)
        self._island_last_title = snap.title
    ...
    def _tick_progress(self):
        ...
        # While actively dragging the seek bar the preview owns the fraction
        if self._seeking:
            return
        scale = self._layout_scale()
    ...
    def send_seek(self, position_sec: float):
        def run_seek():
            import ctypes
            try:
                ctypes.windll.ole32.CoInitializeEx(None, 0)
                ...
                pass
            try:
                safe_run(send_seek_async(position_sec), False)
                # Pull a fresh snapshot so the bar settles on the real position
                QTimer.singleShot(300, self.refresh_media)
            except Exception:
                pass
            finally:
                try:
    ...
    def open_source(self):
        """Bring the current (or last-seen) media app to the foreground."""
        aumid = (self.snapshot.aumid or "").strip() or self._last_seen_aumid
        if not aumid:
            return
        # Debounce: some apps (YT Music PWA) spawn a fresh window per launch,
        # so ignore repeat clicks for a few seconds instead of stacking them.
        now = time.monotonic()
        if now - self._last_open_ts < 4.0:
            return
    ...
    # --- Pin to window -------------------------------------------------
    def _set_on_top_hint(self, on: bool):
        """Add/remove WindowStaysOnTopHint without touching the saved setting."""
        pos = self.pos()
        flags = self.windowFlags()
        if on:
            flags |= Qt.WindowType.WindowStaysOnTopHint
    ...
    def toggle_pin(self):
        if self._pin_target_hwnd:
            self._unpin()
            self._show_volume_feedback("Unpinned")
            return

        target = self._last_ext_hwnd
        if not target or not _hwnd_alive(target):
            self._update_pin_icon()  # also un-checks the menu action
            self._show_volume_feedback("Click a window first, then pin")
            return
        if _hwnd_minimized(target):
            # e.g. pinning from the bare desktop right after minimizing an
            # app — attaching would hide the widget instantly. Refuse instead.
            self._update_pin_icon()
            self._show_volume_feedback("Open a window first, then pin")
            return

        self._pin_target_hwnd = target
        self._pin_target_pid = _hwnd_pid(target)
        self._pin_target_title = _hwnd_title(target)
        # The widget must sit above the pinned window while it's active
        self._pin_forced_on_top = not self.always_on_top
        if self._pin_forced_on_top:
            self._set_on_top_hint(True)
        self._update_pin_icon()
        short = self._pin_target_title[:34] or "window"
        self._show_volume_feedback(f"Pinned to {short}")

    def _unpin(self, auto: bool = False):
        self._pin_target_hwnd = 0
        self._pin_target_pid = 0
        self._pin_target_title = ""
        if self._pin_forced_on_top:
            self._pin_forced_on_top = False
        ...
        self._update_pin_icon()
        if auto:
            self._show_volume_feedback("Pin released (window closed)")

    def _update_pin_icon(self):
        if not hasattr(self, "pin_btn"):
            return
        pinned = bool(self._pin_target_hwnd)
        if hasattr(self, "pin_menu_action"):
            self.pin_menu_action.setChecked(pinned)
        color = self._current_dot_color if pinned else self._current_icon_color
        self.pin_btn.setIcon(make_pin_icon(pinned, color))
        if pinned:
            short = self._pin_target_title[:40] or "window"
            self.pin_btn.setToolTip(f"Pinned to {short} • Click to unpin")
        else:
            self.pin_btn.setToolTip(
                "Pin to the window you were just using — the widget shows and "
                "hides together with it"
            )

    def _pin_tick(self):
        """200ms watcher: track pin candidates and enforce pinned visibility."""
        fg = _fg_hwnd()
        my_pid = os.getpid()

        # Remember the most recent real foreground window that isn't ours —
        # that's what toggle_pin attaches to (clicking us steals focus first).
        # Desktop/taskbar shells are excluded so sitting on the bare desktop
        # never becomes a pin target.
        ...
        if not target:
            return
        if not _hwnd_alive(target):
            self._unpin(auto=True)
            return

        if _hwnd_minimized(target):
            want_visible = False
        else:
            # Visible while the pinned window (any window of its process, so
            # dialogs count) or PulseDock itself is in the foreground.
            fg_pid = _hwnd_pid(fg) if fg else 0
            want_visible = bool(fg) and (
                fg == target or fg_pid in (my_pid, self._pin_target_pid)
            )
    ...
    def hide_to_tray(self):
        # Manual hide overrides the pin — release it so the watcher doesn't
    ...
    # Height of the grab strip along the card's bottom edge for click-to-seek
    SEEK_ZONE_PX = 14
    ...
    def _in_seek_zone(self, local_point) -> bool:
        return self._seek_available() and local_point.y() >= self.height() - self.SEEK_ZONE_PX

    def _point_on_art(self, local_point) -> bool:
        return self.art_label.isVisible() and self.art_label.geometry().contains(local_point)

    def _apply_seek_preview(self, x: int):
        frac = max(0.0, min(1.0, x / max(1, self.width())))
        self._seek_preview_frac = frac
        self.progress_line.set_fraction(frac)
        self.progress_line.show()

    def _update_seek_hover(self):
        self.progress_line.set_hover(self._hovering and self._seek_available())
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            self._did_drag = False
            self._press_on_art = self._point_on_art(local)
            if self._in_seek_zone(local):
                self._seeking = True
                self.progress_line.set_hover(True)
                self._apply_seek_preview(local.x())
                event.accept()
                return
            self._seeking = False
    ...
    def mouseMoveEvent(self, event):
        if event.buttons() & Qt.MouseButton.LeftButton:
            if self._seeking:
                self._apply_seek_preview(event.position().toPoint().x())
                event.accept()
                return
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self._seeking:
            ...
            if dur is not None and dur > 0:
                frac = self._seek_preview_frac
                self.send_seek(frac * dur + self.snapshot.start_sec)
                # Optimistically re-anchor local interpolation to the new spot
                self._pos_base = frac * dur
                self._pos_ts = time.monotonic()
            self._update_seek_hover()
            event.accept()
            return
        if not self._did_drag and self._press_on_art:
            self.open_source()
        elif self._did_drag and self.current_scale_pct == 200:
            # Island stays glued to the top edge; only its x is free
            self._island_snap()
        event.accept()
    ...
    def _update_volume_icon(self, force: bool = False):
        ...
        if self._layout_scale() not in (100, 150):
            return
        vol = self.sys_volume.get_volume()
        muted = self.sys_volume.get_mute()
